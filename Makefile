# ============================================================================
# opencv-rs workspace targets
#
# Mirrors the vmb-rs Makefile's dev-experience (fmt/clippy/deny/shear/taplo/
# typos, per-crate mutants, bench harness). No Nix wiring — opencv-rs's
# only native dep is the optional OpenCV backend via the `opencv` feature.
# ============================================================================
...
# Exclude `opencv` (and any feature that transitively enables it) —
# building --features opencv requires the OpenCV native libraries on the
# host.
rust-test-features:
	@$(RUST_ENV) cargo hack check --feature-powerset --workspace --exclude-features opencv
...
CRATES      := opencv-rs-core opencv-rs-fake opencv-rs-ffi opencv-rs
...
rust-mutants-crate:
	@test -n "$(CRATE)" || (echo "Usage: make rust-mutants-crate CRATE=opencv-rs-core" && exit 1)
	@mkdir -p $(MUTANTS_OUT)
	@$(MUTANTS_TERM_ENV) $(RUST_ENV) cargo mutants --package $(CRATE) \
		--timeout=$(MUTANTS_TIMEOUT) --in-place \
		--output $(MUTANTS_OUT)/$(CRATE) \
...
rust-mutants-file:
	@test -n "$(FILE)" || (echo "Usage: make rust-mutants-file FILE=opencv-rs-core/src/lib.rs" && exit 1)
